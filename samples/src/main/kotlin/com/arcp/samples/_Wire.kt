 ...
 *
 * Each example pretends the SDK already exposes a Python-style `client.envelope(...)`
 * mint + `client.request(...)` round-trip + `client.events()` flow. The real
 * Kotlin SDK reaches v1.0 via `ARCPRuntime` instead; until then, these stubs
 * keep the *protocol* code in each sample readable without forcing every
 * example to re-elide the same five helpers.
 *
 * Every helper is `TODO` — calling it at runtime explodes. The samples are
...
@Suppress("UNUSED_PARAMETER")
public suspend fun ARCPClient.dispatch(envelope: Envelope): MessageId = TODO("v1.0: ARCPClient.send")
