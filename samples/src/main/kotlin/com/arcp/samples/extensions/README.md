...
client.request(client.envelope(type = EXT_TUNE, payload = mapOf(...)))
client.request(client.envelope(type = EXT_GAIN, payload = mapOf(...)))
val capture = client.request(client.envelope(type = EXT_CAPTURE, payload = mapOf(...)))
val audio = client.request(
    client.envelope(type = EXT_DEMODULATE, payload = mapOf(
        "iq_artifact_id" to capture.payloadMap()["artifact_id"],
        ...,
    )),
)
```
...
- `Main.kt` — client side: tune, gain, capture, demodulate, then
