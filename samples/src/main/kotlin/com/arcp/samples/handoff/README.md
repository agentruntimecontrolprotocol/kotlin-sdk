...
```kotlin
val artifact = packageContext(cheap, transcript = ...)   // artifact.put → ref
emitHandoff(cheap, artifactRef = artifact, traceId = traceId)
```
...
- `Main.kt` — open cheap (pinned), ask, escalate if needed.
- `Cheap.kt` — `attempt` + canonical JSON stub.
...
- Deep tier holds the session; cheap closes immediately after
  emitting the handoff (current shape).
