...
```kotlin
val client: ARCPClient = TODO("observer client")
client.open()                                       // subscriptions: true, nothing else
val sub = subscribe(client, target, types = listOf("metric"))
client.events().collect { env ->
    val inner = unwrapEvent(env) ?: return@collect
    sink.handle(inner)
}
```
...
- `Main.kt` — boots three clients in parallel via `coroutineScope`.
- `Sinks.kt` — `StdoutSink`, `SqliteSink`, `OtlpSink`.
