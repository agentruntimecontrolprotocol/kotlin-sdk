...
client.request(
    client.envelope(
        ...
        payload = mapOf(
            "permission" to "host.write",
            "resource" to "host:$host/usr/bin/systemctl/api-gateway",
            "operation" to "restart",
            "reason" to "service is OOMing every 4 minutes",
            "requested_lease_seconds" to 60,
        ),
    ),
    timeoutMs = 120_000,
)
...
- `Main.kt` — opens session, classifies argv, requests leases,
  streams `kind: thought`.
- `Agent.kt` — `LlmStep` + `llmLoop` stub for the Anthropic call.
