...
emitProgress(client, jobId, step = "synthesize")
emitCheckpoint(client, jobId, step = "synthesize")

// resume picks up at the step *after* the last checkpoint
val last = issueResume(client, jobId, afterMessageId, checkpointId)
...
- `Main.kt` — `start_fresh` vs `resume`. `exitProcess(137)` on the
  crash step to demonstrate process death.
- `Steps.kt` — step body stub.

## Variations

- Plug a step-graph checkpointer that doubles to a SQLite store so
